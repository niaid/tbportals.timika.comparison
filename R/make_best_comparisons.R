...
make_best_comparisons <- function(sensitive_predictions, mdr_predictions) {

  sen_summary <- sensitive_predictions$rset_mod_both %>% tidy() %>% group_by(model) %>% summarise(mean_posterior = mean(posterior))
  ...
  mdr_summary <- mdr_predictions$rset_mod_both %>% tidy() %>% group_by(model) %>% summarise(mean_posterior = mean(posterior))
  ...
  sen_comparison = sensitive_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
    filter(grepl(best_timika_sen, contrast) & grepl(best_model_sen, contrast))
  mdr_comparison = mdr_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
