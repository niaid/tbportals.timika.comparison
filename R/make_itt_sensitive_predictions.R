...
#' @title
#' @param cohort_df
make_itt_sensitive_predictions <- function(cohort_df) {

  # Best case scenario for endpoints
  all_cases <- cohort_df$cxr_microscopy_df
  
  # Positive or negative microscopy prediction-------------
  df <- all_cases %>% ungroup() %>%
    mutate(across(where(is.logical), ~as.integer(.x)))
  df %<>% filter(type_of_resistance == "Sensitive")
  ...
  # Consider Still on treatment, Unknown, or Lost to follow up as Cured/Completed
  df <- all_cases %>% ungroup() %>%
    mutate(across(where(is.logical), ~as.integer(.x)))
  df %<>% filter(type_of_resistance == "Sensitive")
  df %<>% select(overallpercentofabnormalvolume:aremediastinallymphnodespresent, matches("_sextant"), outcome, timika_score:nodule, starts_with("qure")) %>%
    select(-ends_with("_num"))
  df %<>% mutate(outcome = factor(ifelse(outcome %in% c("Died", "Failure"), "pos", "neg"), levels = c("neg", "pos")))
