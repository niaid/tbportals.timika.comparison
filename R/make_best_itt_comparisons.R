...
#'
#' @title
#' @param sensitive_itt_predictions
#' @param mdr_itt_predictions
make_best_itt_comparisons <- function(sensitive_itt_predictions,
                                      mdr_itt_predictions) {

  sen_summary <- sensitive_itt_predictions$rset_mod_both %>% tidy() %>% group_by(model) %>% summarise(mean_posterior = mean(posterior))
  ...
  mdr_summary <- mdr_itt_predictions$rset_mod_both %>% tidy() %>% group_by(model) %>% summarise(mean_posterior = mean(posterior))
  ...
  sen_comparison = sensitive_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
    filter(grepl(best_timika_sen, contrast) & grepl(best_model_sen, contrast))
  mdr_comparison = mdr_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
  ...
  sen_outcome_comparison = sensitive_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
    filter(grepl(best_timika_outcome_sen, contrast) & grepl(best_model_outcome_sen, contrast))
  mdr_outcome_comparison = mdr_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
    filter(grepl(best_timika_outcome_mdr, contrast) & grepl(best_model_outcome_mdr, contrast))
  
  sen_both_comparison = sensitive_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
    filter(grepl(best_timika_sen, contrast) & grepl(best_timika_outcome_sen, contrast))
  mdr_both_comparison = mdr_itt_predictions$preproc_diff_both %>%
    summary(size = 0.02) %>%
